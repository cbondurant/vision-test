...
fn frame_to_raw_image(
	...
	let bytes = (frame.height() * frame.width() * 3) as usize;

	let mut buffer = Vec::with_capacity(bytes);
	for i in 0..bytes / 3 {
		let greyscale = data[i * 3] as f64 * 0.299
			+ data[i * 3 + 1] as f64 * 0.587
			+ data[i * 3 + 2] as f64 * 0.114;
		buffer.push(greyscale as u8);
		buffer.push(greyscale as u8);
		buffer.push(greyscale as u8);
	}

	let image =
		glium::texture::RawImage2d::from_raw_rgb(buffer, (new_frame.width(), new_frame.height()));
